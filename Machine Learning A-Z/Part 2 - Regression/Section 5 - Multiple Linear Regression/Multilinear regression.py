...
startup_X = startup_dataset.iloc[:,0:4].values   # dependend variable matrix means that will helo in prediction [rows , columns]
startup_Y = startup_dataset.iloc[:,4].values      # independent variable vector means that need to be predicted [rows , columns]
...
imputer = Imputer(missing_values = 0 , strategy = 'mean' , axis = 0)   # making object of that class
imp_1 = imputer.fit(startup_X[:, 0:3])  # fit above imputer object to our matrix X (we can do this way as well to select column by index no by  imp_1 = imputer.fit(X[:, [1,2]])   )
startup_X[:,  0:3] = imp_1.transform(startup_X[:,  0:3])   # LHS X dataset and RHs imp_object to implement missing values  (   X[:, 1:3] = imp_1.transform(X[:, 1:3])   )
...
X_train , X_test , Y_train, Y_test = train_test_split(startup_X,startup_Y , test_size = 0.25 , random_state=0) #test_size can be 0.3 or 1/3
...
sales = [{'R&D Spend': 50000, 'Administration': 60000, 'Marketing Spend': 3000, 'State': 'Florida'}]
...
#Here we are splitting categorial variable states in column as no of catogories - no of columns
onehotencoder = OneHotEncoder(categorical_features = [3])
x = onehotencoder.fit_transform(x).toarray() # Dummy variable  is created against state
...
profit_pred = regressor.predict(X_test) #sal_test is the predicted model salaries for employees
p = regressor.predict(X_test)


full = startup_X 
profit_pred = regressor.predict(full)
