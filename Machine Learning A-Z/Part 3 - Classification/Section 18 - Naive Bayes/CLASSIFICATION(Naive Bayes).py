...
DT_Y = DT_dataset.iloc[:,4].values



      # independent variable vector means that need to be predicted [rows , columns]
DT_Z= DT_dataset.iloc[:,[1]].values 
DT_Z2= DT_dataset.iloc[:,[2]].values 
DT_Z3= DT_dataset.iloc[:,[3]].values
DT = np.arange(11, 17, 1)
DT.dtype.name
if DT.dtype.name == 'int64' or DT.dtype.name =='float' or DT.dtype.name =='int32':
    print('*****It is Kind of Continous or Binary Number*****')
    one = 1
    zero=0
    if (one and zero in  DT_Y) or ('yes' and 'no' in DT_Y):
        print('it is binary')
    else :
        print('it is continous')
elif DT.dtype.name == 'object' or DT.dtype.name =='str':
    print('it is Catgeorical variable')
...
# 8) Fitting Naive Bayes Classifier to Training Set
